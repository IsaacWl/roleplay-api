...
export default class GroupRequestsController {
  public async index({ request, response }: HttpContextContract) {
    ...
    if (!master) throw new BadRequest('master not provided in query', 422)
  ...
  public async store({ request, response, auth }: HttpContextContract) {
    ...
    const userInGroup = await Group.query()
    ...
    if (userInGroup) throw new BadRequest('user is already in the group', 422)
